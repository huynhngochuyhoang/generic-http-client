...
| Timeout | `@TimeoutMs` | `@ApiRef timeout-ms`, then `resilience.timeout-ms` | `0` explicitly disables the per-request timeout. Network read/write safety nets still apply. |
